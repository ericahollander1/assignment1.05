...
int movepc(dungeon_t *d, int move){
    pair_t nextPair;
    int curx = d->pc.position[dim_x], cury = d->pc.position[dim_y];
    ...
    if(move == 1){
       if(curx > 1 && cury < 20){
        ...
        if(d->map[cury][curx] == ter_stairs_down){
            delete_dungeon(d);
            init_dungeon(d);
            gen_dungeon(d);
            config_pc(d);
        ...
        if(d->map[cury][curx] == ter_stairs_up){
            delete_dungeon(d);
            init_dungeon(d);
            gen_dungeon(d);
            config_pc(d);
